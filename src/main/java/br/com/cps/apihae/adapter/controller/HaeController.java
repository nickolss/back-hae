...
@Tag(
    name = "Hae",
    description = "Endpoints para manipular HAEs (Horas de Atividades Específicas)"
)
public class HaeController {
    ...
    @PostMapping("/create")
    public ResponseEntity<Object> createHae(
        @Valid @RequestBody HaeRequest request
    ) {
        return ResponseEntity.status(HttpStatus.CREATED).body(
            haeFacade.createHae(request)
        );
    }
    ...
    public ResponseEntity<HaeDetailDTO> getHaeById(
        ...
        String userInstitutionId = isGlobalAccessRole(
    ...
    public ResponseEntity<Object> deleteHae(@PathVariable String id) {
        ...
        return ResponseEntity.ok(
            Collections.singletonMap("mensagem", "HAE deletado com sucesso.")
        );
    }
    ...
    public ResponseEntity<List<Hae>> getHaesByEmployeeIdWithLimit(
        ...
        return ResponseEntity.ok(
            haeFacade.getHaesByEmployeeIdWithLimit(employeeId)
        );
    }
    ...
    public ResponseEntity<List<HaeResponseDTO>> getAllHaes(
        ...
        return ResponseEntity.ok(
            haeFacade.getHaesByInstitutionId(scopeInstitutionId)
        );
    }

    @PutMapping("/change-status/{id}")
    public ResponseEntity<Object> changeHaeStatus(
        @PathVariable String id,
        @Valid @RequestBody HaeStatusUpdateRequest request
    ) {
        return ResponseEntity.ok(
            haeFacade.updateHaeStatus(
                id,
    ...
    public ResponseEntity<List<HaeResponseDTO>> getHaesByProfessor(
        ...
        if (authToken == null || authToken.isBlank()) {
            return ResponseEntity.ok(
                haeFacade.getHaesByProfessorId(professorId)
            );
        }
    ...
    public ResponseEntity<?> getHaesByType(
        ...
        List<Hae> filtered = haeFacade
            .getHaesByType(haeType)
            .stream()
            .filter(
                hae ->
                    hae.getInstitution() != null &&
    ...
    public ResponseEntity<Hae> updateHae(
        @PathVariable String id,
        @RequestBody HaeRequest request
    ) {
        return ResponseEntity.status(HttpStatus.OK).body(
            haeFacade.updateHae(id, request)
        );
    }

    @PostMapping("/createHaeAsCoordinator/{coordinatorId}/{employeeId}")
    public ResponseEntity<?> createHaeAsCoordinator(
        @PathVariable String coordinatorId,
        @PathVariable String employeeId,
        @Valid @RequestBody HaeRequest request
    ) {
        return ResponseEntity.status(HttpStatus.CREATED).body(
            haeFacade.createHaeAsCoordinator(coordinatorId, employeeId, request)
        );
    }

    @Operation(
        summary = "Alternar status de visualização",
        description = "Alterna o status 'viewed' de uma HAE (de true para false e vice-versa)."
    )
    ...
    @Operation(
        summary = "Listar HAEs visualizadas",
        description = "Retorna todas as HAEs que já foram visualizadas"
    )
    @GetMapping("/viewed")
    ...
    @Operation(
        summary = "Listar HAEs não visualizadas",
        description = "Retorna todas as HAEs que ainda não foram visualizadas"
    )
    ...
    @Operation(
        summary = "Lista todas as HAEs de uma instituição",
        description = "Retorna uma lista de HAEs baseada no ID da instituição fornecido."
    )
    ...
    public ResponseEntity<List<HaeResponseDTO>> getHaesByInstitution(
        ...
        return ResponseEntity.ok(
            haeFacade.getHaesByInstitutionId(effectiveInstitutionId)
        );
    }

    @Operation(
        summary = "Lista todas as HAEs baseado nos status",
        description = "Retorna uma lista de HAEs baseado no status fornecido."
    )
    ...
    @Operation(
        summary = "Busca avançada de HAEs",
        description = "Filtra HAEs por instituição, curso, tipo, status e visualização. Todos os filtros são opcionais."
    )
    @GetMapping("/search")
    ...
    public ResponseEntity<HaeHoursResponseDTO> getWeeklyHoursByHae(
    ...
    @Operation(
        summary = "Solicitar fechamento de HAE",
        description = "Envia as informações de fechamento de uma HAE baseado no tipo"
    )
    @PostMapping("/request-closure/{haeId}")
    public ResponseEntity<Object> requestClosure(
        @PathVariable String haeId,
    ...
    @Operation(
        summary = "Buscar histórico de fechamento de HAE",
        description = "Retorna todos os registros de fechamento aprovado de uma HAE"
    )
    @GetMapping("/closure-records/{haeId}")
    public ResponseEntity<List<HaeClosureRecordDTO>> getClosureRecords(
        @PathVariable String haeId
    ) {
    ...
    private Employee getAuthenticatedEmployee(String authToken) {
        ...
        return employeeRepository
            .findById(employeeId)
            .orElseThrow(() ->
                new SecurityException("Usuário autenticado não encontrado.")
            );
    }
    ...
    private void assertEmployeeInstitutionScope(
        ...
        Employee targetEmployee = employeeRepository
            ...
            .orElseThrow(() ->
                new IllegalArgumentException("Usuário alvo não encontrado.")
            );

        if (
            targetEmployee.getInstitution() == null ||
            !scopeInstitutionId.equals(targetEmployee.getInstitution().getId())
        ) {
            throw new IllegalArgumentException(
                "Acesso negado para usuários de outra instituição."
            );
        }
    }
}
